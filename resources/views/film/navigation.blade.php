  ...
  <div class="listings-navigation--inner">
    ...
    @if($type == 'weekly')
      <div class="weekly-screenings--navigation">
          ...
          @else
            <button class="weekly-screenings--navigation--trigger button__ghost">{{ Carbon\Carbon::parse($week_commencing)->format('D j F') }} &mdash; {{ Carbon\Carbon::parse($week_ending)->format('D d F') }}</button>
          @endif
        ...
        <nav class="weekly-screenings--week-picker">
          ...
          @for ($i = 2; $i < 8; $i++)
            <a href="/whats-on/weekly/week-{{ $i }}" class="weekly-screenings--week-picker--date @if ($week == $i) current @endif">
              From {{ date("D j F",time() + (($i - 1) * 7) * 86400) }}
            </a>
          @endfor
        </nav>
      </div>
